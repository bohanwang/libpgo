...
public:
  ...
  virtual int saveToBinary(FILE *binaryOutputStream, unsigned int *bytesWritten = nullptr, bool countBytesOnly = false) const = 0;
  ...
  inline Vec3d &getVertex(int element, int vertex) { return vertices[elements[element * numElementVertices + vertex]]; }
  inline const Vec3d &getVertex(int element, int vertex) const { return vertices[elements[element * numElementVertices + vertex]]; }
  inline void getElementVertices(int element, Vec3d *elementVertices) const;  // elementVertices should have size of getNumElementVertices() pre-allocated
  ...
  void getVertexNeighborhood(const std::vector<int> &vertices, std::vector<int> &neighborhood) const;  // get 1-ring of neighboring vtx, including input vtx

  // proximity queries
  virtual int getClosestElement(const Vec3d &pos) const;          // finds the closest element to the given position (using linear scan); distance to a element is defined as distance to its center
  ...
  // if old2NewVertexIDMap is non-null, it also returns a renaming datastructure: old2NewVertexIDMap[big mesh vertex] is the vertex index in the subset mesh
  void setToSubsetMesh(const std::set<int> &subsetElements, int removeIsolatedVertices = 1, std::map<int, int> *old2NewVertexIDMap = nullptr);
  void removeIsolatedVertices(std::map<int, int> *old2NewVertexIDMap = nullptr);
  ...
  // If elements is not nullptr, the closest elements for each target location will be returned in the integer list "*elements" (allocated inside the function)
  // If elements is not nullptr, the function will allocate an integer array *elements, and return the closest element to each target location in it.
  // Returns the number of target points that do not lie inside any element.
  int generateInterpolationWeights(int numTargetLocations, const double *targetLocations, int **vertices, double **weights, double zeroThreshold = -1.0, int **elements = nullptr, int verbose = 0) const;  // this is the "master" function, meant to be typically used to create the interpolant
...
protected:
  ...
  VolumetricMesh(int numVertices, const double *vertices,
    int numElements, int numElementVertices, const int *elements,
...
inline VolumetricMesh::Material::Material(const std::string name_, double density_):
  density(density_)
{
  name = name_;
}
inline VolumetricMesh::Material::Material(const Material &material):
