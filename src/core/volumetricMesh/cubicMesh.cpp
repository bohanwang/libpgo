...
CubicMesh::CubicMesh(int numVertices, const double *vertices,
...
CubicMesh::CubicMesh(const CubicMesh &source):
  VolumetricMesh(source), cubeSize(source.cubeSize), invCubeSize(source.invCubeSize), parallelepipedMode(source.parallelepipedMode)
{
}
...
int CubicMesh::interpolateData(double *vertexData, int numInterpolationLocations, int r, double *interpolationLocations, double *destMatrix, double distanceThreshold) const
{
  ...
  for (int i = 0; i < numInterpolationLocations; i++)  // over all interpolation locations
  {
    ...
    for (int j = 0; j < r; j++) {
      Vec3d data000 = Vec3d(vertexData[ES::ELT(3 * numVertices, 3 * v000 + 0, j)], vertexData[ES::ELT(3 * numVertices, 3 * v000 + 1, j)], vertexData[ES::ELT(3 * numVertices, 3 * v000 + 2, j)]);
      Vec3d data100 = Vec3d(vertexData[ES::ELT(3 * numVertices, 3 * v100 + 0, j)], vertexData[ES::ELT(3 * numVertices, 3 * v100 + 1, j)], vertexData[ES::ELT(3 * numVertices, 3 * v100 + 2, j)]);
      Vec3d data110 = Vec3d(vertexData[ES::ELT(3 * numVertices, 3 * v110 + 0, j)], vertexData[ES::ELT(3 * numVertices, 3 * v110 + 1, j)], vertexData[ES::ELT(3 * numVertices, 3 * v110 + 2, j)]);
      Vec3d data010 = Vec3d(vertexData[ES::ELT(3 * numVertices, 3 * v010 + 0, j)], vertexData[ES::ELT(3 * numVertices, 3 * v010 + 1, j)], vertexData[ES::ELT(3 * numVertices, 3 * v010 + 2, j)]);

      Vec3d data001 = Vec3d(vertexData[ES::ELT(3 * numVertices, 3 * v001 + 0, j)], vertexData[ES::ELT(3 * numVertices, 3 * v001 + 1, j)], vertexData[ES::ELT(3 * numVertices, 3 * v001 + 2, j)]);
      Vec3d data101 = Vec3d(vertexData[ES::ELT(3 * numVertices, 3 * v101 + 0, j)], vertexData[ES::ELT(3 * numVertices, 3 * v101 + 1, j)], vertexData[ES::ELT(3 * numVertices, 3 * v101 + 2, j)]);
      Vec3d data111 = Vec3d(vertexData[ES::ELT(3 * numVertices, 3 * v111 + 0, j)], vertexData[ES::ELT(3 * numVertices, 3 * v111 + 1, j)], vertexData[ES::ELT(3 * numVertices, 3 * v111 + 2, j)]);
      Vec3d data011 = Vec3d(vertexData[ES::ELT(3 * numVertices, 3 * v011 + 0, j)], vertexData[ES::ELT(3 * numVertices, 3 * v011 + 1, j)], vertexData[ES::ELT(3 * numVertices, 3 * v011 + 2, j)]);
...
int CubicMesh::normalCorrection(double *vertexData, int numInterpolationLocations, int r, double *interpolationLocations, double *staticNormals, double *normalCorrection, double distanceThreshold) const
{
  ...
  for (int i = 0; i < numInterpolationLocations; i++)  // over all interpolation locations
  {
    ...
    for (int j = 0; j < r; j++) {
      Vec3d u000 = Vec3d(vertexData[ES::ELT(3 * numVertices, 3 * v000 + 0, j)], vertexData[ES::ELT(3 * numVertices, 3 * v000 + 1, j)], vertexData[ES::ELT(3 * numVertices, 3 * v000 + 2, j)]);
      Vec3d u100 = Vec3d(vertexData[ES::ELT(3 * numVertices, 3 * v100 + 0, j)], vertexData[ES::ELT(3 * numVertices, 3 * v100 + 1, j)], vertexData[ES::ELT(3 * numVertices, 3 * v100 + 2, j)]);
      Vec3d u110 = Vec3d(vertexData[ES::ELT(3 * numVertices, 3 * v110 + 0, j)], vertexData[ES::ELT(3 * numVertices, 3 * v110 + 1, j)], vertexData[ES::ELT(3 * numVertices, 3 * v110 + 2, j)]);
      Vec3d u010 = Vec3d(vertexData[ES::ELT(3 * numVertices, 3 * v010 + 0, j)], vertexData[ES::ELT(3 * numVertices, 3 * v010 + 1, j)], vertexData[ES::ELT(3 * numVertices, 3 * v010 + 2, j)]);

      Vec3d u001 = Vec3d(vertexData[ES::ELT(3 * numVertices, 3 * v001 + 0, j)], vertexData[ES::ELT(3 * numVertices, 3 * v001 + 1, j)], vertexData[ES::ELT(3 * numVertices, 3 * v001 + 2, j)]);
      Vec3d u101 = Vec3d(vertexData[ES::ELT(3 * numVertices, 3 * v101 + 0, j)], vertexData[ES::ELT(3 * numVertices, 3 * v101 + 1, j)], vertexData[ES::ELT(3 * numVertices, 3 * v101 + 2, j)]);
      Vec3d u111 = Vec3d(vertexData[ES::ELT(3 * numVertices, 3 * v111 + 0, j)], vertexData[ES::ELT(3 * numVertices, 3 * v111 + 1, j)], vertexData[ES::ELT(3 * numVertices, 3 * v111 + 2, j)]);
      Vec3d u011 = Vec3d(vertexData[ES::ELT(3 * numVertices, 3 * v011 + 0, j)], vertexData[ES::ELT(3 * numVertices, 3 * v011 + 1, j)], vertexData[ES::ELT(3 * numVertices, 3 * v011 + 2, j)]);
...
void CubicMesh::interpolateGradient(int element, const double *U, int numFields, Vec3d pos, double *grad) const
{
  ...
  for (int j = 0; j < numFields; j++) {
    Vec3d u000 = Vec3d(U[ES::ELT(3 * numVertices, 3 * v000 + 0, j)], U[ES::ELT(3 * numVertices, 3 * v000 + 1, j)], U[ES::ELT(3 * numVertices, 3 * v000 + 2, j)]);
    Vec3d u100 = Vec3d(U[ES::ELT(3 * numVertices, 3 * v100 + 0, j)], U[ES::ELT(3 * numVertices, 3 * v100 + 1, j)], U[ES::ELT(3 * numVertices, 3 * v100 + 2, j)]);
    Vec3d u110 = Vec3d(U[ES::ELT(3 * numVertices, 3 * v110 + 0, j)], U[ES::ELT(3 * numVertices, 3 * v110 + 1, j)], U[ES::ELT(3 * numVertices, 3 * v110 + 2, j)]);
    Vec3d u010 = Vec3d(U[ES::ELT(3 * numVertices, 3 * v010 + 0, j)], U[ES::ELT(3 * numVertices, 3 * v010 + 1, j)], U[ES::ELT(3 * numVertices, 3 * v010 + 2, j)]);

    Vec3d u001 = Vec3d(U[ES::ELT(3 * numVertices, 3 * v001 + 0, j)], U[ES::ELT(3 * numVertices, 3 * v001 + 1, j)], U[ES::ELT(3 * numVertices, 3 * v001 + 2, j)]);
    Vec3d u101 = Vec3d(U[ES::ELT(3 * numVertices, 3 * v101 + 0, j)], U[ES::ELT(3 * numVertices, 3 * v101 + 1, j)], U[ES::ELT(3 * numVertices, 3 * v101 + 2, j)]);
    Vec3d u111 = Vec3d(U[ES::ELT(3 * numVertices, 3 * v111 + 0, j)], U[ES::ELT(3 * numVertices, 3 * v111 + 1, j)], U[ES::ELT(3 * numVertices, 3 * v111 + 2, j)]);
    Vec3d u011 = Vec3d(U[ES::ELT(3 * numVertices, 3 * v011 + 0, j)], U[ES::ELT(3 * numVertices, 3 * v011 + 1, j)], U[ES::ELT(3 * numVertices, 3 * v011 + 2, j)]);
...
{
  double singleMassMatrix[64] = { 0.0370370335876942, 0.0185185167938471, 0.00925925839692354, 0.0185185167938471,
