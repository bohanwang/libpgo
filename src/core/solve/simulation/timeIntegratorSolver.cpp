...
int TimeIntegratorSolver::solve(bool needRenew, ES::VXd& x, [[maybe_unused]] ES::VXd& g,
                                [[maybe_unused]] ES::VXd& lambda, const ES::VXd& xlow, const ES::VXd& xhi,
...
int TimeIntegratorSolver::solveDirect(ES::VXd& x, ES::VXd& g, ES::VXd& lambda, const ES::VXd& xlow, const ES::VXd& xhi,
                                      const ES::VXd& clow, const ES::VXd& chi,
                                      std::shared_ptr<const PotentialEnergy>     energy,
                                      std::shared_ptr<const ConstraintFunctions> constraints, int nIter, double eps,
                                      int verbose, const char* solverConfigFilename,
                                      TimeIntegratorSolverOption solverOption) {
    int solverRet = 0;

    if (constraints) {
        if (solverOption == TimeIntegratorSolverOption::SO_IPOPT)
            solverRet = EnergyOptimizer::minimize(x, energy, xlow, xhi, lambda, g, constraints, clow, chi,
            ...
            solverRet = EnergyOptimizer::minimizeUsingKnitro(x, energy, xlow, xhi, lambda, g, constraints, clow, chi,
                                                             nIter, eps, verbose, solverConfigFilename);
    } else {
        if (solverOption == TimeIntegratorSolverOption::SO_IPOPT)
            solverRet = EnergyOptimizer::minimize(x, energy, xlow, xhi, EnergyOptimizer::SolverType::ST_IPOPT, nIter,
                                                  eps, verbose);
        else if (solverOption == TimeIntegratorSolverOption::SO_NEWTON)
            solverRet = EnergyOptimizer::minimize(x, energy, xlow, xhi, EnergyOptimizer::SolverType::ST_NEWTON, nIter,
                                                  eps, verbose);
        else if (solverOption == TimeIntegratorSolverOption::SO_KNITRO)
            solverRet = EnergyOptimizer::minimizeUsingKnitro(x, energy, xlow, xhi, lambda, g, nullptr, ES::VXd(),
                                                             ES::VXd(), nIter, eps, verbose, solverConfigFilename);
    }
