...
ImplicitBackwardEulerTimeIntegrator::ImplicitBackwardEulerTimeIntegrator(
    ...
    double stiffnessDampingCoeff, double t, int numIter, double eps_)
...
std::shared_ptr<const NonlinearOptimization::PotentialEnergy> ImplicitBackwardEulerTimeIntegrator::getInternalEnergy()
    const {
    return eulerEnergy;
}
