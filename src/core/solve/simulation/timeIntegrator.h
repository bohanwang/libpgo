...
public:
    TimeIntegrator(const EigenSupport::SpMatD&                                   massMatrix,
                   ...
                   double massDampingCoeff, double stiffnessDampingCoeff, double timestep, int numIter = 0,
                   double eps = 1e-5);
    ...
    void addImplicitForceModel(std::shared_ptr<ConstraintPotentialEnergies::PotentialEnergyAligningMeshConnectivity> fm,
                               double kd = -1, double md = 0);
    void clearImplicitForceModel();

    void addGeneralImplicitForceModel(std::shared_ptr<NonlinearOptimization::PotentialEnergy> fm, double kd = -1,
                                      double md = 0);
    ...
    virtual void addConstraints(std::shared_ptr<NonlinearOptimization::ConstraintFunctions> constt,
                                const int*                                                  equalities = nullptr);
    virtual void addConstraints(std::shared_ptr<NonlinearOptimization::ConstraintFunctions> constt,
    ...
    void setFixedVertices(const std::vector<int>& fixedVertices, EigenSupport::ConstRefVecXd fixedRestPosition,
                          EigenSupport::ConstRefVecXd fixedPosition, int isIndexSorted = 1);
...
protected:
    ...
    std::vector<std::shared_ptr<ConstraintPotentialEnergies::PotentialEnergyAligningMeshConnectivity>>
                                      additionalForceModels;
    ...
    enum ImplicitModelType { IMT_ELASTIC, IMT_SAME_TOPOLOGY, IMT_GENERAL };
